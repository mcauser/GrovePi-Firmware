...
void TM1637Display::begin(uint8_t pinClk, uint8_t pinDIO)
{
	// Copy the pin numbers
	m_pinClk = pinClk;
	m_pinDIO = pinDIO;
...
{
	// 0-7
	//m_brightness = (brightness & 0x07) + 0x08;
	//brightness = brightness ^ (brightness & -(brightness < 0));  // brightness = max(brightness, 0)
	//brightness = 7 ^ ((brightness ^ 7) & -(brightness < 7));     // brightness = min(brightness, 7)
