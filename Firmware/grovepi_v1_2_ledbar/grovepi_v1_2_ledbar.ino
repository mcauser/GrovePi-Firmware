...
void loop()
{
  ...
  if(index==4 && flag==0)
  {
    ...
    // Led bar - set level (0-10)
    // [50, pin, level, reverse]
    if(cmd[0]==50)
    {
      // clock pin is always next to data pin
      ledbar.begin(cmd[1]+1, cmd[1]);  // clock, data
      
      // green to red, or red to green?
      if (cmd[3]==1) {
        ledbar.setLevelReverse(cmd[2]);
      }
      else {
        ledbar.setLevel(cmd[2]);
      }
    }
    
    // Led bar - set single led (0-10, 0-1)
    // [51, pin, index, bool]
    if(cmd[0]==51)
    {
      ledbar.begin(cmd[1]+1, cmd[1]);  // clock, data
      ledbar.setSingleLed(cmd[2], cmd[3]);  // index, bool
    }
    
    // Led bar - led index bit (light em up with binary)
    // [52, pin, byte1, byte2]
    // to light all 10x leds, we need to use 10 bits, and since there are 8 bits to a byte, we need to use 2 bytes
    if(cmd[0]==52)
    {
      ledbar.begin(cmd[1]+1, cmd[1]);  // clock, data
      ledbar.ledIndexBit((cmd[2] * 256) + cmd[3]);
    }
  }
}
